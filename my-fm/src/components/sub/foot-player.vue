				...
				<div class="content-box">
					...
					<div class="control-box">
						...
						<div class="progress_box">
							...
							<div class="progress_outer" ref="progress" @touchstart.prevent="progressTouchStart" @touchmove.prevent="progressTouchMove" @touchend.prevent="progressTouchEnd">
								<div class="progress">
						...
						<div class="player_contolrol_box">
							...
							<div class="player_mode">
								<span v-show="playerMode == 1" class="iconfont icon-shunxubofang" @click.stop="changePlayMode(2)"></span>
								<span v-show="playerMode == 2" class="iconfont icon-danquxunhuan" @click.stop="changePlayMode(3)"></span>
								<span v-show="playerMode == 3" class="iconfont icon-suiji" @click.stop="changePlayMode(1)"></span>
							</div>
							<!--上一首 下一首 播放 暂停-->
		...
		<div v-if="footerShow">
			<transition name="footer">
				...
				<footer class="footer-box" @click="toUp" v-show="!fullScreen&& playerList.length > 0">
					...
					<span class="smallNext iconfont icon-xiayishou" @click.stop="changeRadio(playerInfo.id,1)"></span>
					<span class="smallPlaylist iconfont icon-yinleliebiao" @click.stop="changePopUPshow"></span>
				</footer>
			</transition>
			<footer class="footer-box" @click.stop="$toast('先选择播放内容啦 o(∩_∩)o')" v-show="playerList.length <= 0">
...
export default {
	...
	methods: {
		...
		changeRadio(id, num) {
			...
			let index = this.playerList.findIndex(item => item.id == id) + num;
		...
		//随机播放
		randomRadio() {
			let anyIndex = Math.floor(Math.random() * this.playerList.length);
			let nextId = this.playerList[anyIndex].id;
			this.$store.commit("setMediaUrlId", nextId);
		},
		//播放和暂停的控制
		playClick() {
