    ...
    <van-sticky :offset-top="top" z-index="10" @scroll="scroll()">
      <div class="operation" ref="operation">
        <div>
          ...
          <span class="paly_all" @click="all">播放全部</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getSpeakerInfo() {
      let res = await sc(
        "GET",
        "http://yiapi.xinli001.com/fm/diantai-detai.json",
        {
          params: {
            id: this.id,
    ...
    //获取主播的电台列表
    async getspeakRadiolist(id) {
      let res = await sc(
        "GET",
        "http://yiapi.xinli001.com/fm/diantai-jiemu-list.json",
        {
          params: {
            offset: this.offset,
            limit: this.limit,
    ...
    async getRadioOn() {
      let res = await sc(
        "GET",
        "http://yiapi.xinli001.com/fm/diantai-check-guanzhu.json",
        {
          params: {
            diantai_id: this.id,
            key: this.$store.state.key,
    ...
    async getDoGuanzhu() {
      let res = await sc(
        "GET",
        "http://yiapi.xinli001.com/fm/diantai-toggle-guanzhu.json",
        {
          params: {
            diantai_id: this.id,
            key: this.$store.state.key,
    ...
    all() {
      this.$store.commit("setFullScreen", true);
      this.$store.commit("setMediaUrlId", this.speakRadiolist[0].id);
      let newlist = JSON.parse(JSON.stringify(this.speakRadiolist)); //利用json转换深拷贝
      this.$store.commit("setPlayerList", newlist);
    }
  }
