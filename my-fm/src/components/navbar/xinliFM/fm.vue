		...
		<div class="main-box">
			...
			<div class="radio_list">
				...
				<div class="change" @click="rotateChageIcon()">
					<span :style="{'transform':`translateY(-50%) rotate(${rotate}deg)`}" class="rotate_class changeicon iconfont icon-icon_refresh" ref="changeOne"></span>
			...
			<div class="radio_list">
				<van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
					<div class="radio_item" v-for="(item, index) in newSelectList" :key="index" @click="toPlayerPage(item.id)">
						<div class="cover">
							<img :src="item.cover" />
							<span class="play iconfont icon-Album-Play"></span>
						</div>
...
import sc from "../../../tools/sc-axios.js";
...
export default {
			...
			recommendBodanList: [
				{
					...
					num: "32.2万",
					cover:
						"http://image.xinli001.com/20150707/14130647beae354a278dbf.jpg"
				},
				{
					name: "失恋",
					title: "失恋专辑,失恋治愈节目",
					num: "45.5万",
					cover:
						"http://image.xinli001.com/20141117/1738460ae93e8ba100e462.jpg"
				},
				{
					name: "旅行",
					title: "十一特辑：旅行",
					num: "12.5万",
					cover:
						"http://image.xinli001.com/20140930/1631005b0ad44c3934c5d1.jpg"
				},
				{
					name: "北京不仅是一座城",
					title: "北京不仅是一座城",
				...
				},
				{
					...
					num: "45.9万",
					cover:
						"http://image.xinli001.com/20140107/1522130ff36fd23a4b2e6b.jpg"
				},
				{
					name: "听言集",
					title: "听言集",
					num: "9.8万",
					cover:
						"http://image.xinli001.com/20150407/10285377b6ae1536003283.jpg"
				},
				{
					name: "夏天",
					title: "遇见夏天",
					num: "49.8万",
					cover:
						"http://image.xinli001.com/20150417/152606ac2648abe9fc9509.jpg"
				},
				{
					name: "三颗小豆子奇遇记",
					title: "三颗小豆子奇遇记",
					num: "6.8万",
					cover:
						"http://image.xinli001.com/20150616/1439464197f778e7716ae5.jpg"
				}
			]
		};
	},
	...
	methods: {
		...
		async getRondomList() {
			...
			let res = await sc(
				"GET",
				"http://yiapi.xinli001.com/fm/newfm-list.json",
				{
					params: {
						offset: random,
						limit: this.limit,
		...
		async getHotRadioList() {
			...
			if (res.data.code === 0) {
				...
				let newData = [];
				for (let i = 0; i < 3; i++) {
					let ranNum = Math.floor(Math.random() * (data.length - i));
					newData.push(data[ranNum]);
					data.splice(ranNum, 1);
				}
				this.HotRadioList = newData;
			}
		},
		rotateChageIcon() {
		...
		radioOperate(obj) {
			...
			this.$store.commit("setRadioObj", obj);
		},
		async getNewList() {
			let res = await sc(
				"GET",
