<template>
	<div class="comment-container" ref="commentcontainer">
		...
		<van-pull-refresh v-model="isLoading" @refresh="onRefresh">
			<div class="radio_box" ref="radiobox">
				...
				<van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
					<div ref="commentBox" v-for="(item, index) in commentList" :key="index" :class="{'comment_box_bg':showNum == index&&show ? true : false,'comment_box':true}" @click="showCommentControl($event,index)">
						<div :style="{'left':positionLift}" ref="showMessageBox" class="show_message_box" v-show="showNum == index&&show ? true : false">
		...
		<footer class="do_comment">
			<input @focus="inputFocus" ref="commentInput" class="comment_input" type="text" @blur="changeSend" :placeholder="placeholderMsg" v-model="content" />
	...
	methods: {
		...
		showCommentControl(e, index) {
			...
			this.$nextTick(() => {
				...
				let maxLeft =
					commentBox[index].offsetWidth -
					showMessageBox[index].offsetWidth;
				if (boxLeft < -7) {
					boxLeft = -7;
		...
		async getCommentList() {
			let res = await sc(
				"GET",
				"http://yiapi.xinli001.com/fm/comment-latest-list.json?",
				{
					params: {
						offset: this.offset,
